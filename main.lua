...
local function initUtils()
	local utils = {}
	local camera = game["Workspace - Client"].Camera.ViewportSize
...
local CircleClick = function(Button)
	...
	circle.Position = UDim2.new(0, game["Players - Client"].LocalPlayer:GetMouse().X - Button.AbsolutePosition.X, 0, game["Players - Client"].LocalPlayer:GetMouse().Y - Button.AbsolutePosition.Y);
...
local LocalPlayer = game["Players - Client"].LocalPlayer
...
function UILibrary.Window:Enabled()
	self.MainUI.Parent.Enabled = not self.MainUI.Parent.Enabled
end
...
function UILibrary.Section:ColorPicker(sett, callback)
	...
	functions.setValue = function(clr)
		...
		currentclr = toHex(clr)

		callback(currentclr)
		element.Text.Label.Text = currentclr
	end
